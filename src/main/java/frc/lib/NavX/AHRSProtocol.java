...
public class AHRSProtocol extends IMUProtocol {
    ...
    public static int decodeAHRSUpdate(byte[] buffer,
        ...
        if ((buffer[offset + 0] == PACKET_START_CHAR) &&
            ...
            u.mag_field_norm_ratio = decodeProtocolUnsignedHundredthsFloat(buffer,
                    offset + AHRS_UPDATE_CAL_MAG_NORM_RATIO_VALUE_INDEX);
            ...
            /*
             * AHRSPosUpdate: Quaternions are signed int (16-bit resolution); divide by
             * 16384 to yield +/- 2 radians
             */
    ...
    public static int decodeAHRSPosUpdate(byte[] buffer,
        ...
        if ((buffer[offset + 0] == PACKET_START_CHAR) &&
            ...
            u.compass_heading = decodeProtocolUnsignedHundredthsFloat(buffer,
                    offset + AHRSPOS_UPDATE_HEADING_VALUE_INDEX);
            u.altitude = decodeProtocol1616Float(buffer, offset + AHRSPOS_UPDATE_ALTITUDE_VALUE_INDEX);
            u.fused_heading = decodeProtocolUnsignedHundredthsFloat(buffer,
                    offset + AHRSPOS_UPDATE_FUSED_HEADING_VALUE_INDEX);
            u.linear_accel_x = decodeProtocolSignedThousandthsFloat(buffer,
                    offset + AHRSPOS_UPDATE_LINEAR_ACCEL_X_VALUE_INDEX);
            u.linear_accel_y = decodeProtocolSignedThousandthsFloat(buffer,
                    offset + AHRSPOS_UPDATE_LINEAR_ACCEL_Y_VALUE_INDEX);
            u.linear_accel_z = decodeProtocolSignedThousandthsFloat(buffer,
                    offset + AHRSPOS_UPDATE_LINEAR_ACCEL_Z_VALUE_INDEX);
    ...
    public static int decodeAHRSPosTSRawUpdate(byte[] buffer,
        ...
        if ((buffer[offset + 0] == PACKET_START_CHAR) &&
            ...
            u.linear_accel_x = decodeProtocol1616Float(buffer,
                    offset + AHRSPOS_TS_RAW_UPDATE_LINEAR_ACCEL_X_VALUE_INDEX);
            u.linear_accel_y = decodeProtocol1616Float(buffer,
                    offset + AHRSPOS_TS_RAW_UPDATE_LINEAR_ACCEL_Y_VALUE_INDEX);
            u.linear_accel_z = decodeProtocol1616Float(buffer,
                    offset + AHRSPOS_TS_RAW_UPDATE_LINEAR_ACCEL_Z_VALUE_INDEX);
            ...
            u.mpu_temp = decodeProtocolSignedHundredthsFloat(buffer,
                    offset + AHRSPOS_TS_RAW_UPDATE_MPU_TEMP_VAUE_INDEX);
            /* AHRSPosTSUpdate: Quaternions are 16.16 format (32-bit resolution). */
            ...
            u.gyro_bias_updated = ((decodeBinaryInt16(buffer,
                    offset + AHRSPOS_TS_RAW_UPDATE_GYRO_BIAS_UPDATED_BOOL_INDEX) != 0) ? true : false);
            ...
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    d.mag_xform[i][j] = decodeProtocol1616Float(buffer,
                            offset + MAG_XFORM_1_1_VALUE_INDEX + (i * 6) + (j * 2));
                }
            }
